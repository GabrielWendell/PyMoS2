...
class FluidVisualiser:
    ...
    def animate_1D(self, quantity, folder='default', extent=[0, 1], anim_fps='auto', showDeviations=True, showParams=True, height=6, aspect=1.3, title='', save=False, anim_time='auto', video_fps=30, video_name='auto', snapshots=None, units={}):
        ...
        def update(i):
            ...
            # Update text
            textbox1.set_text('Tempo: {}\ndt = {:.2g} {}'.format(time_displayer(self.t), dt_avg, time_unit))

            if showDeviations:

                devi_text = ''
                if self.has_arr['rho']: devi_text += 'Desvio de Massa: {:.3f} %%'.format(100*(np.sum(self.arrs['rho'].astype('f8'))/rho_tot0 - 1.0))
                if self.has_arr['e']: devi_text += '\nDesvio de Energia: {:.3f} %%'.format(100*(np.sum(self.arrs['e'].astype('f8'))/e_tot0 - 1.0))
    ...
    def animate_2D(self, quantity, matrixLike=True, folder='default', extent=[0, 1, 0, 1], anim_fps='auto', showDeviations=True, showParams=True, showQuiver=True, quiverscale=1, N_arrows=20, interpolation='none', cmap='seismic', height=6, aspect='equal', cbar_aspect=0.02, title='', save=False, anim_time='auto', video_fps=30, video_name='auto', backgrounds=None, snapshots=None, units={}):
        ...
        ax.set_xlabel('Distância Horizontal' + ('' if extent_unit_x == '' else ' [{}]'.format(extent_unit_x)))
        ax.set_ylabel('Distância Vertical' + ('' if extent_unit_z == '' else ' [{}]'.format(extent_unit_z)))
    ...
    def __get_quiver(self, ax, N_arrows, quiverscale, showQuiver, arrowscale_unit):
        ...
        if showQuiver:
            ...
            if arrowscale_unit != '':
                ...
                arrowtext = 'Escala Vetorial = {:g} {}'.format(arrscale/(self.z[-1, 0] - self.z[0, 0]), arrowscale_unit)

                ax.text(0.965, 1.005, arrowtext, color='k', fontsize=self.fontsize, horizontalalignment='right', 
    ...
    def __prepare_text(self, ax, showExtent=True):
        ...
        textbox1 = ax.text(0.01, 0.97, '', color = 'white', fontsize=self.fontsize, horizontalalignment='left', 
                                           verticalalignment='top', transform=ax.transAxes)

        textbox2 = ax.text(0.99, 0.97, '', color='white', fontsize=self.fontsize, horizontalalignment='right', 
                                          verticalalignment='top', transform=ax.transAxes)

        textbox3 = ax.text(0.01, 0.03, '', color='white', fontsize=self.fontsize, horizontalalignment='left', 
                                           verticalalignment='bottom', transform=ax.transAxes)
